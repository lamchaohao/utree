...
- (void)transitionFromCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<TYTabPagerBarCellProtocol> *)toCell animate:(BOOL)animate {
    ...
    void (^animateBlock)(void) = ^{
        if (fromCell) {
            fromCell.titleLabel.font = self.normalTextFont;
...
//            fromCell.transform = CGAffineTransformMakeScale(self.selectFontScale, self.selectFontScale);
        ...
        if (toCell) {
            toCell.titleLabel.font = self.normalTextFont;
            toCell.titleLabel.textColor = self.selectedTextColor ? self.selectedTextColor : self.normalTextColor;
            //注释掉，免得缩来缩去，烦死个人
