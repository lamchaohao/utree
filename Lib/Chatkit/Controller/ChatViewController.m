...
{
    [super viewDidLoad];
    self.title = self.strChatterName;//标题
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"data"
                                                                    style:UIBarButtonItemStyleDone target:self action:@selector(receiveMessage)];
...
- (KeyboardView *)chatBar
{
    if (!_chatBar)
    {
        _chatBar = [[KeyboardView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, kMinHeight)];
        //(self.navigationController.navigationBar.isTranslucent ? 0 : iPhone_Top_NavH)
        [_chatBar setSuperViewHeight:ScreenHeight -iPhone_Safe_BottomNavH];
        [_chatBar setSuperViewWidth:[UIScreen mainScreen].bounds.size.width];
