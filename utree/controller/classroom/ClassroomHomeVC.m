...
-(void)showLeftView:(UIButton *)sender{
    LeftDrawerView *drawerView = [[LeftDrawerView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
    [[[UIApplication sharedApplication]keyWindow]addSubview:drawerView];
    

    [UIView animateWithDuration:0.3 animations:^{
        CGRect rect = [drawerView.contentView frame];
        CGRect tableViewRect = [drawerView.tableView frame];
        //
        rect.origin.x = 0;
        tableViewRect.origin.x = 0;
//        tableViewRect.origin.y = 12;
        drawerView.backgroundColor =  [UIColor colorWithRed:0 green:0 blue:0 alpha:0.4];
        [drawerView.contentView setFrame:rect];
        [drawerView.tableView setFrame:tableViewRect];
...
//    self.view = rootView;
//    rootView.insetsPaddingFromSafeArea=UIRectEdgeAll;
...
//    self.titleView.myTop = 0;
//    self.titleView.myLeading = 0;
...
#pragma mark 菜单按钮展开显示
//-(void)expandMenuButton
//{
//    MenuExpandView *drawerView = [[MenuExpandView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight) andEx:_mainMenuBtn.center];
//    [[[UIApplication sharedApplication]keyWindow]addSubview:drawerView];
//
//}
