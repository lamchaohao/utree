...
- (void)setMomentsBodyFrames{
    ...
    CGFloat nameY = iconY;
    //    CGFloat nameW = 120;
    CGSize nameSize = [self.notice.poster sizeWithAttributes:circleCellNameattributes];
    self.bodyNameFrame = (CGRect){{nameX,nameY},nameSize};
    
    //时间
    CGFloat timeX = nameX;
    CGFloat timeY = CGRectGetMaxY(self.bodyNameFrame) + circleCellMargin * 0.5;
    CGSize timeSize = [self.notice.postTime boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:circleCellTimeattributes context:nil].size;
    ...
    CGSize textSize = [self.notice.noticeDetail boundingRectWithSize:CGSizeMake(textW, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:circleCellTextattributes context:nil].size;
    self.bodyTextFrame = (CGRect){{textX,textY},textSize};
    
    //图片 (判断是否有图片)
    if ([self.notice.photoUrls count] != 0) {
        CGFloat photosX = circleCellMargin;
        CGFloat photosY = CGRectGetMaxY(self.bodyTextFrame) + circleCellMargin;
        CGFloat photosW = textW;
        if ([self.notice.photoUrls count] > 3) {
...
- (void)setCellHeight{
    self.cellHeight = CGRectGetMaxY(self.momentsToolBarFrame)+circleCellToolBarHeight;
}
