...
//
#define LVRecordFielName @"lvRecord_test.caf"
...
//    NSTimer *timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(updateImage) userInfo:nil repeats:YES];
//    [timer fire];
//    self.timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(updateImage) userInfo:nil repeats:YES];
...
//    NSLog(@"%f", result);
    ...
    NSLog(@"%s, timer isValid=%d ",__func__,[self.timer isValid]);
...
- (void)stopRecording {
    if ([self.recorder isRecording]) {
        ...
        NSLog(@"recordTool stop recording------");
    }
}
...
- (AVAudioRecorder *)recorder {
    if (!_recorder) {
        ...
        NSLog(@"fileURLWithPath= %@", filePath);
