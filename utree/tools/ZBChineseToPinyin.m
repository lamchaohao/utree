...
@interface ZBChineseToPinyin()
// 用将需要排序的对象封装在ZBChineseToPinyin对象中，包含排序的字符串，对象，首字母三个属性
// 进行比较的字符串
...
@implementation ZBChineseToPinyin
#define AbnormalLetter @"#" // 特殊字符统一合并后的 显示字符
...
+ (NSMutableArray *)indexArray:(NSArray *)stringArr {
    ...
    for (NSString* object in tempArray) {
        NSString *pinyin = [((ZBChineseToPinyin*)object).pinYin substringToIndex:1];
        // 不同
...
+ (NSMutableArray *)letterSortArray:(NSArray *)stringArr {
    ...
    for (NSString *object in tempArray) {
        ...
        NSString *pinyin = [((ZBChineseToPinyin*)object).pinYin substringToIndex:1];
        NSString *string = ((ZBChineseToPinyin*)object).string;
...
+ (NSMutableArray *)returnSortChineseArrar:(NSArray *)stringArr {
    ...
    for(int i = 0; i < [stringArr count]; i++) {
        ...
        ZBChineseToPinyin *chineseString = [[ZBChineseToPinyin alloc]init];
        ...
        // 这里递归过滤指定字符串   RemoveSpecialCharacter
        chineseString.string = [ZBChineseToPinyin removeSpecialCharacter:chineseString.string];
        //        ZLLog(@"string====%@",chineseString.string);
...
+ (NSMutableArray *)sortObjectArray:(NSArray *)objectArray Key:(NSString *)key {
    ...
    NSString *tempString;
    // 拼音分组
    for (ZBChineseToPinyin* object in tempArray) {
    ...
    if (ZLLog == 1) { // 1、打印函数耗时   0、不打印
        CFAbsoluteTime useTime = (CFAbsoluteTimeGetCurrent() - startTime);
        NSLog(@"ZBChineseToPinyin user time in %f ms", useTime *1000.0);
    }
...
"dsfnsfpzxhdwztdmpptflzzbzdmZLqjrsdzsqzsqxbdgcpzswdwcsqzgmdhzxmwwfybpngphdmjthzsmmbgzmbzjcfzhfcbbnmq"
...
"mjyyhfllqllxfdyhabxggnywZLllsdldllbjcyxjznlhljdxyyqytdlllbngpfdfbbqbzzmdpjhgclgmjjpgaehhbwcqxajhhhz"
...
"cnrxswgbtgygbwhswdwrfhwytjjxxxjyzyslphZLyyxhydqpxshxyxgskqhywbdddpplcjlhqeewjgsyykdpplfjthkjltcyjhh"
...
"yxwshctxrljgjzhzcyyyjltkttntmjlzclzzayyoczlrlbszywjytsjyhbyshfjlykjxxtmzyyltxxypslqyjzyzZLnhmymdyyl"
