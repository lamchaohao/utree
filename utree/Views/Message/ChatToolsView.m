...
@interface ChatToolsView()<UITextViewDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,Mp3RecorderDelegate,ChatMoreViewDelegate,ChatMoreViewDataSource,ChatFaceViewDelegate>
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (self.bottomHeight<1||self.bottomHeight==kFunctionViewHeight) {//键盘收回或者是显示表情和工具栏
    NSLog(@"self.bottomHeight=%f,superViewHeight=%f",self.bottomHeight,self.superViewHeight);
        addBarFrame.origin.y = self.superViewHeight - self.bottomHeight - addBarFrame.size.height;
    }else{//键盘弹起
        addBarFrame.origin.y = ScreenHeight- self.bottomHeight- addBarFrame.size.height;
    }
...
- (void)faceViewSendFace:(NSString *) strFaceName
{
    ...
    else if ([strFaceName isEqualToString:@"发送"])
    {
        ...
        [self showViewWithType:ChatToolViewShowFace];
    }
    else
    {
...
- (void)endInputing
{
    [self showViewWithType:ChatToolViewShowNothing];
}
...
- (void)longPressForRecord:(UILongPressGestureRecognizer *)press
{
    ...
    switch (press.state)
    {
        case UIGestureRecognizerStateBegan:
        {
            ...
            _voiceRecordButton.backgroundColor = [UIColor myColorWithHexString:@"#E0E0E0"];
...
- (void)showViewWithType:(ChatToolViewShowType)showType
{
    //显示对应的View
    [self showVoiceView:showType == ChatToolViewShowVoice && self.voiceButton.selected];
    [self showFaceView:showType == ChatToolViewShowFace && self.faceButton.selected];
    [self showMoreView:showType == ChatToolViewShowMore && self.moreButton.selected];
    
    switch (showType)
    {
        case ChatToolViewShowNothing:
        case ChatToolViewShowVoice:
        {
        ...
        case ChatToolViewShowMore:
        case ChatToolViewShowFace:
        {
            self.inputText = self.textView.text;
...
{
    self.inputText = self.textView.text;
    ChatToolViewShowType showType = button.tag;
    ...
    if (!button.selected)
    {
        showType = ChatToolViewShowKeyboard;
...
- (void)sendTextMessage:(NSString *)strText
{
    ...
    [self setFrame:CGRectMake(0, self.superViewHeight - self.bottomHeight - kMinHeight, self.superViewWidth, kMinHeight) animated:NO];
    [self showViewWithType:ChatToolViewShowKeyboard];
}
...
- (ChatFaceView *)faceView
{
    if (!_faceView)
    {
        _faceView = [[ChatFaceView alloc] initWithFrame:CGRectMake(0, ScreenHeight, self.superViewWidth, kFunctionViewHeight)];
...
- (ChatMoreView *)moreView
{
    if (!_moreView)
    {
        _moreView = [[ChatMoreView alloc] initWithFrame:CGRectMake(0, ScreenHeight, self.superViewWidth, kFunctionViewHeight)];
...
- (UIButton *)voiceButton
{
    if (!_voiceButton)
    {
        ...
        _voiceButton.tag = ChatToolViewShowVoice;
...
- (UIButton *)voiceRecordButton
{
    if (!_voiceRecordButton) {
        ...
        [_voiceRecordButton setTitleColor:[UIColor myColorWithHexString:@"#808080"] forState:UIControlStateNormal];
        [_voiceRecordButton setTitle:@"按住 说话" forState:UIControlStateNormal];
...
- (UIButton *)moreButton
{
    if (!_moreButton)
    {
        ...
        _moreButton.tag = ChatToolViewShowMore;
...
- (UIButton *)faceButton
{
    if (!_faceButton)
    {
        ...
        _faceButton.tag = ChatToolViewShowFace;
...
- (void)setFrame:(CGRect)frame animated:(BOOL)animated
{
    ...
    NSLog(@"keyboardView setFrame,frame=(%f,%f)",frame.origin.y,frame.size.height);
...
- (void)setSuperViewHeight:(CGFloat)superViewHeight
{
    ...
    NSLog(@"setSuperViewHeight=%f",superViewHeight);
}
