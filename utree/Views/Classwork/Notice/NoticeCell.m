...
-(void)setupUI
{
    ...
    _timeLabel.leftPos.equalTo(_headView.rightPos).offset(6);
...
//
    ...
    _titleLabel.frame = self.noticeViewModel.titleTextFrame;
    CGRect oldTitleFrame = _titleLabel.frame;
    _titleLabel.frame = (CGRect){oldTitleFrame.origin,self.noticeViewModel.titleTextFrame.size};
    ...
    _detailLabel.frame = (CGRect){oldFrame.origin,self.noticeViewModel.detailTextFrame.size};
    ...
    if (self.noticeViewModel.notice.picList.count!= 0) {
        ...
        // 设置图片缩略图数组
        self.photosView.thumbnailUrls = self.noticeViewModel.notice.thumnails;
        // 设置图片原图地址
    ...
    if (self.noticeViewModel.notice.link&&self.noticeViewModel.notice.link.length>0) {
